...
const cloudPhotoBucket = "checkpoint-photos";
const appLocale = "en";
...
function normalizePoint(point, index = 0) {
  return {
    ...
    photos: normalizePhotos(point.photos).map((photo) => photoMetadata(photo)),
...
function normalizePhotos(photos) {
  ...
  return photos
    .filter((photo) => photo?.id || photo?.src || photo?.url || photo?.path)
    .map((photo) => ({
      id: photo.id || makeId(),
...
function cloudPhotoPath(photo) {
  const safeName = (photo.name || "photo.jpg").replace(/[^a-z0-9._-]+/gi, "-");
  return `${photo.id}-${safeName}.jpg`;
}

function cloudPhotoUrl(path) {
  return `${supabaseProjectUrl}/storage/v1/object/public/${cloudPhotoBucket}/${encodeURIComponent(path)}`;
}
...
async function uploadPhotoToCloud(photo) {
  ...
  const path = photo.path || cloudPhotoPath(photo);
  const response = await fetch(`${supabaseProjectUrl}/storage/v1/object/${cloudPhotoBucket}/${encodeURIComponent(path)}`, {
    method: "POST",
    headers: supabaseHeaders({
      "Content-Type": "image/jpeg",
      "x-upsert": "true",
    }),
    body: dataUrlToBlob(photo.src),
  });
  ...
  return {
    ...
    name: photo.name,
    path,
    url: cloudPhotoUrl(path),
    src: photo.src,
  };
}

async function deletePhotoFromCloud(photo) {
  if (!photo?.path) return;

  await fetch(`${supabaseProjectUrl}/storage/v1/object/${cloudPhotoBucket}/${encodeURIComponent(photo.path)}`, {
    method: "DELETE",
    headers: supabaseHeaders({ "Content-Type": "application/json" }),
    body: JSON.stringify({ prefixes: [photo.path] }),
  }).catch(() => {});
}
...
async function addPhotos(files) {
  ...
  try {
    ...
    const photos = await Promise.all(compressedPhotos.map((photo) => uploadPhotoToCloud(photo)));

    await Promise.all(photos.map((photo) => savePhotoRecord(point.id, photo)));
    point.photos = [
      ...normalizePhotos(point.photos),
      ...photos.map((photo) => photoMetadata(photo)),
    ];
    saveState();
    render();
  } catch (error) {
...
async function removePhoto(photoId) {
  ...
  const photo = normalizePhotos(point.photos).find((item) => item.id === photoId);
  await deletePhotoRecord(photoId);
  await deletePhotoFromCloud(photo);
  point.photos = normalizePhotos(point.photos).filter((item) => item.id !== photoId);
  saveState();
  render();
}
